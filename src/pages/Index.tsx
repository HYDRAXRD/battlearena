    ...
    <div className="relative min-h-screen bg-background overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        {hasName && state.screen === 'battle' && (
          <motion.div key="battle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative">
        ...
        {hasName && state.screen === 'victory' && (
          ...
          >
            <div className="bg-black/80 border-2 border-game-teal rounded-xl p-8 text-center max-w-sm w-full shadow-2xl">
              <div className="text-4xl mb-4">🎉</div>
              <h2 className="font-pixel text-lg text-game-teal mb-2">VICTORY!</h2>
              <p className="font-pixel text-[8px] text-white/60 mb-4">
                {victoryEnemy.name} Defeated!
              </p>
              <div className="font-pixel text-[8px] text-yellow-400 mb-6 flex items-center justify-center gap-1">
                <HydrToken size={12} /> +{victoryEnemy.tokenReward} HYDR
              </div>
              <button
                onClick={isLastBattle ? () => setScreen('leaderboard') : nextBattle}
                className="font-pixel text-[10px] md:text-sm py-4 px-8 bg-game-teal text-black rounded border-b-4 border-black/30 hover:brightness-110 active:border-b-0 active:translate-y-1 transition-all w-full mb-3"
              >
        ...
        {hasName && state.screen === 'defeat' && (
          <motion.div
            ...
            className="relative z-10 min-h-screen flex items-center justify-center px-4"
          >
            <div className="bg-black/80 border-2 border-red-600 rounded-xl p-8 text-center max-w-sm w-full shadow-2xl">
              <div className="text-4xl mb-4">💀</div>
              <h2 className="font-pixel text-lg text-red-500 mb-2">DEFEATED!</h2>
              <p className="font-pixel text-[8px] text-white/60 mb-6">
